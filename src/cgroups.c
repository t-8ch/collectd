...
static int read_cgroups_table(const char *type, const char *dirname,
  ...
  ssnprintf(abs_path, sizeof(abs_path), "%s/%s/%s", dirname,
            cgroup_name, table_name);
  fh = fopen(abs_path, "r");
  if (fh == NULL) {
    char errbuf[1024];
...
static int read_cpuacct_procs (const char *dirname, char const *cgroup_name,
    void *user_data) {
  return read_cgroups_table("cpu", dirname, cgroup_name, "cpuacct.stat", DS_TYPE_DERIVE, user_data);
} /* int read_cpuacct_procs */

__attribute__ ((nonnull(1, 2)))
static int read_memory_procs (const char *dirname, char const *cgroup_name,
    void *user_data) {
  return read_cgroups_table("memory", dirname, cgroup_name, "memory.stat", DS_TYPE_GAUGE, user_data);
...
static int read_cgroups_root (const char *dirname, const char *filename,
 ...
 */
__attribute__ ((nonnull(1, 2)))
static int read_cpuacct_root(const char *dirname, const char *filename, void *user_data) {
 ...
 */
__attribute__ ((nonnull(1, 2)))
static int read_memory_root(const char *dirname, const char *filename, void *user_data) {
  ...
  for (i = 0; i < STATIC_ARRAY_SIZE(settings); i++) {
    ...
    if (0 == settings[i].found)
      WARNING("cgroups plugin: Unable to find cgroup "
          "mount-point with the \"%s\" option.", settings[i].controller);
  }
